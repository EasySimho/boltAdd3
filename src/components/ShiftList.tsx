...
const generateExcel = async () => {
  try {
    ...
    // Fetch all shifts if not already loaded
    ...
    if (shifts.length === 0) {
      ...
      shiftsData = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as Shift[];
    }
    ...
    for (const shift of shiftsData) {
      // Skip shifts without participants
      if (!shift.participants || Object.keys(shift.participants).length === 0) {
        continue;
      }

      // Format date from YYYY-MM-DD to a readable format
      const formattedDate = format(parseISO(shift.date), "dd/MM/yyyy", { locale: it });

      // Group participants by role
      const participantsByRole: Record<string, string[]> = {};

      for (const [userId, participant] of Object.entries(shift.participants)) {
        if (!participantsByRole[participant.role]) {
          participantsByRole[participant.role] = [];
        }
        participantsByRole[participant.role].push(participant.name);
      }

      // For each role, create a string with all participants
      let participantsText = '';
      for (const [role, names] of Object.entries(participantsByRole)) {
        participantsText += `${names.join(', ')} (${role}); `;
      }

      // Remove trailing semicolon and space
      participantsText = participantsText.trim().replace(/;\s*$/, '');
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Turni Adunata Alpini');
    ...
    const fileName = `Turni_Adunata_Alpini_${format(new Date(), 'dd-MM-yyyy')}.xlsx`;

    // Save file
    saveAs(blob, fileName);
  } catch (error) {
    console.error('Errore durante la generazione del file Excel:', error);
    alert('Si è verificato un errore durante la generazione del file Excel. Riprova più tardi.');
  } finally {
    setLoading(false);
  }
};
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center">
              ...
              <button
                ...
                disabled={loading}
                className="flex items-center px-3 py-2 text-sm font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
                title="Scarica tutti i turni in Excel"
              >
                {loading ? (
                  <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5 sm:mb-6">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto">
          <button
            ...
            disabled={isFirstDay}
            className={`p-2 sm:p-3 rounded-lg transition-colors border ${isFirstDay
          ...
          <button
            ...
            disabled={isLastDay}
            className={`p-2 sm:p-3 rounded-lg transition-colors border ${isLastDay
                ? 'text-gray-300 border-gray-200 cursor-not-allowed'
                : 'hover:bg-gray-50 text-gray-700 border-gray-300 active:bg-gray-100'
              }`}
